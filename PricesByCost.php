...
$Title = _('View or Update Prices Based On Costs');
...
if (isset($_POST['submit']) OR isset($_POST['update'])) {
	...
	mostrarAyuda( _('Items in') . ' ' . $CategoryText . ' ' . _('With Prices') . ' ' . $Comparator . '' . $_POST['Margin'] . ' ' . _('times') . ' ' . _('Cost in Price List') . ' ' . $SalesTypeRow['sales_type']);
	...
	if ($numrow > 0) { //the number of prices returned from the main prices query is
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ($myrow['price'] != 0){
				$CurrentGP = (($myrow['price']/1)-$Cost)*100 / ($myrow['price']/1);
			} else {
				$CurrentGP = 0;
			}
		...
		echo '</tbody>
			<tfoot>
				<tr>
			<td class="number" colspan="4"><input type="submit" name="update" value="' . _('Update') .
			 '" onclick="return confirm(\'' . 
			 _('If the prices above do not have a commencement date as today, this will create new prices with commencement date of today at the entered figures and update the existing prices with historical start dates to have an end date of yesterday. Are You Sure?') . '\');" /></td>
	...
	mostrarAyuda( _('Prices can be displayed based on their relation to cost'));
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post" class="col-12">';
    echo '<div>';
    echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
    echo '<table class="selection">';
	...
	echo '</td>
		...
		<tr>
			<td>' . _('Currency') . ':</td>
			<td><input type="hidden" name="CurrCode" value="MXN"/>Peso mexicano
			</td></tr>';
	echo '</table>
		<br />
