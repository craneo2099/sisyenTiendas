...
/*	echo '<p class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
	...
	echo '<DIV class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
		'/images/inventory.png" title="', // Icon image.
		_('Inventory Item'), '" /> ', // Icon title.
		_('Inventory Item'), ': ', $StockID, ' - ', $myrow['description'], ' ', $ItemStatus, '</DIV>';// Page title.
	...
	echo '<DIV class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
		'/images/inventory.png" title="', // Icon image.
		_('Select An Item'), '" /> ', // Icon title.
		_('Select An Item'), '</DIV>',// Page title.
		$TableHead,
		'<tr>',
	...
	if ($ListCount > 0) {
		...
		while (($myrow = DB_fetch_array($SearchResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			echo '<tr class="striped_row linkRow" data-href="'.$onclick.'">
				<td>' . $ItemStatus . '</td>
			<td><input type="submit" id="Select'.$RowIndex.'" name="Select" value="' . $myrow['stockid'] . '" /></td>
			<td>'.$StockImgLink.'</td>
			<td title="'. $myrow['longdescription'] . '">' . $myrow['description'] . '</td>
			<td class="number">' . $qoh . '</td>
			<td>' . $myrow['units'] . '</td>
			<td class="nestedLink"><a target="_blank" href="' . $RootPath . '/StockStatus.php?who=selectproduct?StockID=' . urlencode($myrow['stockid']).'">' . _('View') . '</a></td>
