...
if ($NoOfGRNs >0){
	...
	for ($i=1;$i<=$NoOfGRNs;$i++) {
		...
		if ($ControlledRow[0]==1) { /*Then its a controlled item */
			...
			while ($SerialStockMoves = DB_fetch_array($GetStockMoveResult)){
				...
				$LeftOvers = $pdf->addText($FormDesign->OrderNumber->x,$Page_Height-$FormDesign->OrderNumber->y,$FormDesign->OrderNumber->FontSize,_('P/O').': ' . $myrow['orderno']);
				$PageNumber++;
    ...
    $pdf->OutputD( $_SESSION['CompanyRecord']['coyname']. '_EtiquetaAC_' . $GRNNo . '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
