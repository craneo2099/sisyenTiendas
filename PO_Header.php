...
if ($_SESSION['RequireSupplierSelection'] == 1 or !isset($_SESSION['PO' . $identifier]->SupplierID) or $_SESSION['PO' . $identifier]->SupplierID == '') {
	...
	$SupplierID=$_POST['Select'];
	echo '<tr>
				<td>' . _('Supplier Contact') . ':</td>
				<td><select name="SupplierContact">';

	$SQL = "SELECT contact FROM suppliercontacts WHERE supplierid='" . $SupplierID . "'";
	$SuppCoResult = DB_query($SQL);
	...
	echo '</select><a href="' . $RootPath . '/SupplierContacts.php?SupplierID=' . $SupplierID . '"
		target="_blank">',_('Add'),'</a><a href="#" onclick="ReloadForm(form1.SearchSuppliers)">
		',_('Update'),'
		</a> </td>
		</tr>
		<tr>
