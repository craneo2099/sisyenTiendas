...
// gather the invoice data
	...
	if (isset($_GET['Email'])){ //email the invoice to address supplied
		...
		$FileName = $_SESSION['reports_dir'] . '/' . $companyName . '_' . _($InvOrCredit) . '_' . $FromTransNo . '.pdf';
		...
		if($_SESSION['SmtpSetting'] == 0){
			$mail->setFrom($companyName . ' <' . $_SESSION['CompanyRecord']['email'] . '>');
		...
		$pdf->OutputD($companyName . '_' . _($InvOrCredit) . '_' . $FromTransNo . '.pdf');
