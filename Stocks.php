...
if (isset($StockID) && $StockID != '' && $InputError == 0){
    if($New){
		...
		prnMsg(_('No se encontró el artículo').' '.$StockID,'warn');
	}
...

{
	...
	<div class="form-group row">
		<label for="StockID" class="col-sm-4 col-form-label">' .  _('Item Code'). ':</label>';

	if ($New || $StockID=='' or isset($_POST['UpdateCategories'])) {

	/*If the page was called without $StockID passed to page then assume a new stock item is to be
	entered show a form with a part Code field other wise the form showing the fields with the 
	existing entries against the part will show for editing with only a hidden StockID field. 
	New is set to flag that the page may have called itself and still be entering a new part, 
	in which case the page needs to know not to go looking up details for an existing part*/
		echo '<div class="col-sm-6">
					<input type="text" class="' . (in_array('StockID',$Errors) ?  'inputerror' : '' ) . ' form-control"
						data-type="no-illegal-chars" autofocus="autofocus" required="required"  value="'.$StockID.'" 
						name="StockID" id="StockID" size="20" maxlength="20" 
						aria-describedby="stockCodeHelp" 
						placeholder="'._('alpha-numeric only').'" />
					<small id="stockCodeHelp" class="form-text text-muted">'._('Input the stock code, the following characters are prohibited:') . ' \' &quot; + . &amp; \\ &gt; &lt;</small>
				</div>
				<div class="col-sm-2"> 
					<button name="findBarCode" type="submit" class="btn btn-primary" formnovalidate>Buscar</button>
				</div>';
	} elseif (!isset($_POST['UpdateCategories']) AND $InputError!=1 AND $accionId>=1) { // Must be modifying an existing item and no changes made yet
		...
		<span>' . $StockID . '</span><input type="hidden" name ="StockID" id ="StockID" value="' . $StockID . '" />
		</div>';
...
echo '
			<div class="form-group row">
				<label for="Description" class="col-sm-4 col-form-label">' .  _('Part Description'). ':</label>
				<div class="col-sm-8">
				<input type="text" class="' . (in_array('Description',$Errors) ?  'inputerror' : '' ) . ' form-control"
					data-type="no-illegal-chars" ' . 
					($New==0?'autofocus="autofocus"':'') . ' required="required"  value="' . stripslashes($Description) . '" 
					name="Description" id="Description"  size="52" maxlength="50" />
				</div>
			</div>';
...
foreach ($ItemDescriptionLanguagesArray as $LanguageId) {
	if ($LanguageId!=''){
		...
		echo '<div class="form-group row">
				<label for="'. $PostVariableName . '" class="col-sm-4 col-form-label">' . $LanguagesArray[$LanguageId]['LanguageName'] . ' ' .  _('Description'). ':</label>
				<div class="col-sm-8">
				<input type="text" class="form-control"
					value="' . $_POST[$PostVariableName] . '" 
					name="'. $PostVariableName . '" id="'. $PostVariableName . '"  size="52" maxlength="50"  />
				</div>
			</div>';
	}
}
...
echo '<div class="form-group row">
		<label for="LongDescription" class="col-sm-4 col-form-label">' .  _('Part Description'). ' (' . _('long') . '):</label>
		<div class="col-sm-8">
		<textarea class="' . (in_array('LongDescription',$Errors) ?  'texterror' : '' ) . ' form-control"
			name="LongDescription" id="LongDescription" cols="40" rows="3"
			>' . stripslashes($LongDescription) . '</textarea></div>
	</div>';
...
foreach ($ItemDescriptionLanguagesArray as $LanguageId) {
	if ($LanguageId!=''){
		...
		echo '<div class="form-group row">
				<label for="'. $PostVariableName . '" class="col-sm-4 col-form-label">' . $LanguagesArray[$LanguageId]['LanguageName'] . ' ' . _('Long Description') . ':</label>
				<div class="col-sm-8">
				<textarea class="form-control"
					name="'. $PostVariableName . '" id="'. $PostVariableName . '" cols="40" rows="3"
					>' . stripslashes(AddCarriageReturns($_POST[$PostVariableName])) . '</textarea>
					</div></div>';
	}
}
...
echo '<div class="form-group row">
		...
		<div class="col-sm-8">
			...
			<div class="form-check">
				<input type="checkbox" name="ClearImage" id="ClearImage" class="form-check-input"
					value="1" >
				<label for="ClearImage" class="form-check-label">'._('Clear Image').'</label> 
			</div>
		</div>
	</div>';

$globo=glob(
		$_SESSION['part_pics_dir'] . '/' . $StockID . '.{' .
				implode(",", $SupportedImgExt) . '}', GLOB_BRACE
		);
...
if ($StockImgLink!=_('No Image')) {
	echo '<div>' . _('Image') . '<br />' . $StockImgLink . '</div>';
}
...
{
	echo '<div class="form-group row">
			<label for="CategoryID" class="col-sm-4 col-form-label">' . _('Category') . ':</label>
			<div class="col-sm-8">
				<select name="CategoryID" id="CategoryID" class="form-control"
				onchange="ReloadForm(ItemForm.UpdateCategories)">';
...
echo '<div class="form-group row">
	<label for="EOQ" class="col-sm-4 col-form-label">' . _('Economic Order Quantity') . ':</label>
	<div class="col-sm-8">
	<input type="text"  class="' . (in_array('EOQ',$Errors) ?  'texterror' : '' ) . ' form-control number"
		name="EOQ" id="EOQ" size="12" maxlength="10" 
		value="' . locale_number_format($_POST['EOQ'],'Variable') . '" />
	</div></div>';

echo '<div class="form-group row">
		<label for="Volume" class="col-sm-4 col-form-label">' . _('Packaged Volume (metres cubed)') . ':</label>
		<div class="col-sm-8">
		<input type="text" class="' . (in_array('Volume',$Errors) ?  'texterror' : '' ) . ' form-control number"
		 name="Volume" id="Volume" size="12" maxlength="10" 
		 value="' . locale_number_format($_POST['Volume'],'Variable') . '" />
	</div></div>';

echo '<div class="form-group row">
		<label for="GrossWeight" class="col-sm-4 col-form-label">' . _('Packaged Gross Weight (KGs)') . ':</label>
		<div class="col-sm-8">
		<input type="text" class="' . (in_array('GrossWeight',$Errors) ?  'texterror' : '' ) . ' form-control number"
		name="GrossWeight" id="GrossWeight" size="12" maxlength="10" 
		value="' . locale_number_format($_POST['GrossWeight'],'Variable') . '" />
	</div></div>';

echo '<div class="form-group row">
		<label for="NetWeight" class="col-sm-4 col-form-label">' . _('Net Weight (KGs)') . ':</label>
		<div class="col-sm-8">
		<input type="text" class="' . (in_array('NetWeight',$Errors) ?  'texterror' : '' ) . ' form-control number" 
		name="NetWeight" id="NetWeight" size="12" maxlength="10" 
		value="' . locale_number_format($_POST['NetWeight'],'Variable') . '" /></div></div>';

{//peso neto
	echo '<div class="form-group row">
	<label for="NetWeight" class="col-sm-4 col-form-label">' . _('Units of Measure') . ':</label>
	<div class="col-sm-8">
	<select name="Units" id="NetWeight" class="' . (in_array('Description',$Errors) ?  'selecterror' : '' ) . ' form-control">';
	...
	while( $UOMrow = DB_fetch_array($UOMResult) ) {
		if (isset($_POST['Units']) AND $_POST['Units']==$UOMrow['unitname']){
			echo '<option selected="selected" value="' . $UOMrow['unitname'] . '">' . _($UOMrow['unitname']) . '</option>';
		} else {
			echo '<option value="' . $UOMrow['unitname'] . '">' . _($UOMrow['unitname'])  . '</option>';
		}
	}
...
{
	echo '<div class="form-group row">
	<label for="MBFlag" class="col-sm-4 col-form-label">' . _('Assembly, Kit, Manufactured or Service/Labour') . ':</label>
	<div class="col-sm-8">
	<select name="MBFlag" id="MBFlag" class="form-control">';
...
{
	echo '<div class="form-group row">
		<label for="Discontinued" class="col-sm-4 col-form-label">' . _('Current or Obsolete') . ':</label>
		<div class="col-sm-8">
...
{
	echo '<div class="form-group row">
			<label for="Controlled" class="col-sm-4 col-form-label">' . _('Batch, Serial or Lot Control') . ':</label>
			<div class="col-sm-8"> 
			<select name="Controlled" id="Controlled" class="form-control">';
...
{
	echo '<div class="form-group row">
	<label for="Serialised" class="col-sm-4 col-form-label">' . _('Serialised') . ':</label>
	<div class="col-sm-8">
	<select name="Serialised" id="Serialised" 
	class="' . (in_array('Serialised',$Errors) ?  'selecterror' : '' ) . ' form-control"
	aria-describedby="SerialisedHelp">';
	...
	echo '</select><small id="SerialisedHelp" class="form-text text-muted">' . _('Note') . ', ' . _('this has no effect if the item is not Controlled') . '</small>
	</div></div>';
}
if ($_POST['Serialised']==1 AND $_POST['MBFlag']=='M'){
	echo '<div class="form-group row">
		<label for="NextSerialNo" class="col-sm-4 col-form-label">' . _('Next Serial No (>0 for auto numbering)') . ':</label>
		<div class="col-sm-8">
		<input type="text" name="NextSerialNo" id="NextSerialNo" size="15" maxlength="15" 
			class="' . (in_array('NextSerialNo',$Errors) ?  'inputerror' : '' ) . ' form-control"
			value="' . $_POST['NextSerialNo'] . '" /></div></div>';
} else {
	echo '<input type="hidden" name="NextSerialNo" value="0" />';
}
{
	echo '<div class="form-group row">
			<label for="Perishable" class="col-sm-4 col-form-label">' . _('Perishable') . ':</label>
			<div class="col-sm-8">
			<select name="Perishable" id="Perishable" class="form-control">';
...
echo '<div class="form-group row">
		<label for="DecimalPlaces" class="col-sm-4 col-form-label">' . _('Decimal Places for display Quantity') . ':</label>
		<div class="col-sm-8">
		<input type="text"  class="form-control number" name="DecimalPlaces" id="DecimalPlaces" size="1" maxlength="1" 
		value="' . $_POST['DecimalPlaces'] . '" /></div></div>';
...
echo '<div class="form-group row">
		<label for="BarCode" class="col-sm-4 col-form-label">' . _('Bar Code') . ':</label>
		<div class="col-sm-8">
		<input type="text" name="BarCode" id="BarCode" size="22" maxlength="20" 
		class="' . (in_array('BarCode',$Errors) ?  'inputerror' : '' ) . ' form-control"
		value="' . $BarCode . '" /></div></div>';
...
echo '<div class="form-group row">
<label for="DiscountCategory" class="col-sm-4 col-form-label">' . _('Discount Category') . ':</label>
<div class="col-sm-8">
<input type="text" name="DiscountCategory" id="DiscountCategory" class="form-control" size="2" maxlength="2" value="' . $DiscountCategory . '" /></div></div>';

echo '<div class="form-group row">
<label for="TaxCat" class="col-sm-4 col-form-label">' . _('Tax Category') . ':</label>
<div class="col-sm-8">
<select name="TaxCat" id="TaxCat" class="form-control">';
...
echo '<div class="form-group row">
		<label for="PanSize" class="col-sm-4 col-form-label">' . _('Pan Size') . ':</label>
		<div class="col-sm-8">
		<input  class="form-control number" id="PanSize" maxlength="6" name="Pansize" size="6" 
			type="text" value="' . locale_number_format($_POST['Pansize'],0) . '" 
			aria-describedby="PanSizeHelp"/>
		<small id="PanSizeHelp" class="form-text text-muted">' . _('Order multiple. It is the minimum packing quantity.') . '</small>
	</div></div>
	<div class="form-group row">
		<label for="ShrinkageFactor" class="col-sm-4 col-form-label">' . _('Shrinkage Factor') . ':</label>
		<div class="col-sm-8">
		<input  class="form-control number" id="ShrinkageFactor" maxlength="6" name="ShrinkFactor" size="6"
		type="text" value="' . locale_number_format($_POST['ShrinkFactor'],'Variable') . '"
			aria-describedby="ShrinkageFactorHelp" />
		<small id="ShrinkageFactorHelp" 
		class="form-text text-muted">' . _('Amount by which an output falls short of the estimated or planned output.') . '</small>
	</div></div>';
