...
if (isset($_POST['PrintPDF'])){
	...
	while ($InventoryValn = DB_fetch_array($InventoryResult)){
		...
		if ($_POST['DetailedReport']=='Yes'){
			...
			$LeftOvers = $pdf->addTextWrap(423,$YPos,15,$FontSize,_($InventoryValn['units']),'left');
...
/*Print out the grand totals */
	...
	$pdf->OutputD('InformeValoracionInv_' . ($_POST['DetailedReport']=='Yes'?'detalle':'resumen').'_'.Date('Y-m-d') . '.pdf');
	$pdf->__destruct();
	...
	while ($InventoryValn = DB_fetch_row($InventoryResult)) {
		...
		foreach ($InventoryValn as $key => $value) {
			if(!empty($value))
				$InventoryValn[$key]=_($value);
		}
		$CSVListing .= implode('","', $InventoryValn) . '"' . "\n";
	}
	header('Content-Encoding: UTF-8');
    header('Content-type: text/csv; charset=UTF-8');
    header("Content-disposition: attachment; filename=InformeValoracionInv_" .  Date('Y-m-d')  .'.csv');
