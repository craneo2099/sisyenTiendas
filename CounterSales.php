...
$cssList = '/css/'. $_SESSION['Theme']. '/css/datasheets.css';
...
$tabidx=1;
echo  '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '" id="SelectParts" class="col-12" method="post">
...
/* If enter is pressed on the quick entry screen, the default button may be Recalculate */
 if (isset($_POST['SelectingOrderItems'])
	...
	$AlreadyWarnedAboutCredit = false;
	$i=1;
	$entriesQty=contarPrefijos(array_keys ($_POST),'part_');
	$entriesIds=array();
	while ($i<=$entriesQty
...
*/
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $OrderLine) {
		...
		if ($OrderLine->QOHatLoc < $OrderLine->Quantity AND ($OrderLine->MBflag=='B' OR $OrderLine->MBflag=='M')) {
			...
			$RowStarter = '<tr class="stock-deficiency">';
		} else {
		...
		<td><input class="number" tabindex="<?=strval($tabidx++)?>" type="text" 
				name="Quantity_<?= $OrderLine->LineNumber ?>" required="required" 
				onchange="$('#ProcessSale').attr('disabled', true);"
				size="6" maxlength="6" value="<?= locale_number_format($OrderLine->Quantity,$OrderLine->DecimalPlaces) ?>" />
<?php
		...
		$isEntryDiscount=false;
		if (in_array($_SESSION['PageSecurityArray']['OrderEntryDiscountPricing'], $_SESSION['AllowedPageSecurityTokens'])) {
			$isEntryDiscount=true;
			?>
			<td>
				...
				<input class="number" type="text" name="viewPrice_<?= $OrderLine->LineNumber ?>" 
					...
					"/></td>
				<td>
					<input class="number" type="text" name="Discount_<?= $OrderLine->LineNumber ?>" 
						required="required" size="5" maxlength="4"
						onchange="$('#ProcessSale').attr('disabled', true);"
						value="<?=locale_number_format(($OrderLine->DiscountPercent * 100),2) ?>" /></td>
				<td>
					<input class="number" type="text" name="GPPercent_<?= $OrderLine->LineNumber ?>" 
						required="required" size="3" maxlength="40" 
						onchange="$('#ProcessSale').attr('disabled', true);"
						value="<?=locale_number_format($OrderLine->GPPercent,2)?>" /></td>
				<?php
		} else {
	...
	$subTotalTax=$CurrItem->total+$TaxTotal;
	$paymentAdjustment=locale_number_format(
		(getPaymentAdjunstment($_POST['PaymentMethod'])/-100) * ($subTotalTax)
		,$currDecimals);
	$totalTax=$subTotalTax+$paymentAdjustment;
	$formTTax=locale_number_format($totalTax,$currDecimals);
	if($paymentAdjustment!=0){
		?>
		<tr class="striped_row">
			<td colspan="<?=$isEntryDiscount?8:6?>" class="number"><b><?=_('Sub Total') ?></b></td>
			<td colspan="2"></td>
			<td class="number"><?=locale_number_format($subTotalTax,$currDecimals) ?></td>
			<td ></td>
		</tr>
		<tr class="striped_row">
	...
	<tr class="striped_row">
		...
		<td class="number"><?=locale_number_format($CurrItem->total,$currDecimals)?></td>
		<td class="number"><?=locale_number_format($TaxTotal,$currDecimals)?></td>
		<td id="total" class="number"><?=$formTTax ?></td>
		<td ></td>
	</tr>
	</table>
	<input type="hidden" name="TaxTotal" value="<?=$TaxTotal?>" />
	<input type="hidden" name="LastTotalHdn" id="LastTotalHdn" value="<?=$formTTax?>" />
	<input type="hidden" name="hdnAdjustment" id="hdnAdjustment" value="<?=$paymentAdjustment?>" />
<?php
	...
	<tr>
		...
		<td>
			<?php
			$attsPayment="onchange=\"$('#ProcessSale').attr('disabled', true);\" ";
			getpaymentMethodSelect($_POST['PaymentMethod']??2,$attsPayment);
			?>
		</td>
	</tr>
<?php
	...
	if($_POST['AmountGiven']>$totalTax){
		$_POST['AmountPaid']=$formTTax;
	}else{
		$_POST['AmountPaid']=$_POST['AmountGiven'];
	}
	?>
			<tr>
			...
			<tr>
				...
				title="<?= _('Enter the amount paid by the customer, this must equal the amount of the sale') ?>" 
				maxlength="12" size="12" value="<?= $_POST['AmountPaid']?>" /></td>
			</tr>
	...
	if (!isset($_POST['ProcessSale'])) {
		echo '<br />
				<div class="centre">
					...
					<input type="submit" name="ProcessSale" id="ProcessSale" value="' . _('Process The Sale') . '" />
...
if (isset($_POST['ProcessSale']) AND $_POST['ProcessSale'] != '') {
	...
	$txttl=filter_number_format($_POST['LastTotalHdn']);
	$raund=round($txttl,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
	...
	/* Insert the tax totals for each tax authority where tax was charged on the invoice */
		...
		/*Insert Sales Analysis records */
			...
			if (!is_null($MyRow)&&$MyRow[0]>0) {  /*Update the existing record that already exists */
...
if (!isset($_POST['ProcessSale'])) {
	...
	// Find the quantity in stock at location
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td><input class="number" tabindex="'.strval($tabidx++).'" type="text" ' . ($i==0?'autofocus="autofocus"':'') . ' size="6" required="required" name="OrderQty%s" value="0" />
			...
			echo '<td class="centre" colspan="8"><input type="hidden" name="SelectingOrderItems" value="1" />
			<input tabindex="'.strval($tabidx++).'" type="submit" value="'._('Add to Sale').'" /></td>';
			echo '</table>';
		} //end of if Frequently Ordered Items > 0
		echo '</div>';
		
	echo '</form>';
	echo  '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '" id="search" class="col-12" method="post">
	<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	echo '<div>';
		...
		echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . _('Search for Items') . '</p>
			...
			<table class="selection">
				<tr>
					<td><b>' . _('Select a Stock Category') . ': </b><select tabindex="'.strval($tabidx++).'" name="StockCat">';
		...
		echo '</select></td>
				...
				<input tabindex="'.strval($tabidx++).'" type="text" autofocus="autofocus" name="Keywords" size="20" maxlength="25" value="';
        if (isset($_POST['Keywords'])) echo $_POST['Keywords'];
        echo '" /></td>
				<td align="right"><b> ' . _('OR') . ' </b><b>' . _('Enter extract of the Stock Code') . ':</b>
				<input tabindex="'.strval($tabidx++).'" type="text" name="StockCode" size="15" maxlength="18" value="';
        if (isset($_POST['StockCode'])) echo $_POST['StockCode'];
        echo '" /></td>
			</tr>
			<tr>
				<td class="centre" colspan="1"><input tabindex="'.strval($tabidx++).'" type="submit" name="Search" value="' . _('Search Now') . '" /></td>
				<td class="centre" colspan="1"><input tabindex="'.strval($tabidx++).'" type="submit" name="QuickEntry" value="' . _('Use Quick Entry') . '" /></td>
			</tr>
		</table>
		<br />
		</div>';
	...
	echo '</form>';
	echo  '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '" id="result" class="col-12" method="post">
	<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	
	echo '<input type="hidden" name="PartSearch" value="' .  _('Yes Please') . '" />';
	echo '<input type="submit" value="'._('Add to Sale').'" style="display:none"/>';
	echo '<div>';
	// Add some useful help as the order progresses
		if (isset($SearchResult)) {
			...
			echo '<table class="table1">';
			echo '<tr>
					<td><input type="hidden" name="PreviousList" value="'.strval($Offset-1).'" /><input tabindex="'.strval($tabidx++).'" type="submit" name="Previous" value="'._('Prev').'" /></td>
					<td class="centre" colspan="6"><input type="hidden" name="SelectingOrderItems" value="1" /><input tabindex="'.strval($tabidx++).'" type="submit" value="'._('Add to Sale').'" /></td>
					<td><input type="hidden" name="NextList" value="'.strval($Offset+1).'" /><input tabindex="'.strval($tabidx++).'" type="submit" name="Next" value="'._('Next').'" /></td>
				</tr>
				<tr>
					<th>' . _('Code') . '</th>
		   			<th>' . _('Description') . '</th>
			...
			while ($MyRow=DB_fetch_array($SearchResult)) {
				...
				printf('<tr class="striped_row">
						...
						<td class="number">%s</td>
						<td class="number">%s</td>
						<td><input class="number"  tabindex="'.strval($tabidx++).'" type="text" size="6" ' . ($i==0?'autofocus="autofocus"':'') . ' name="OrderQty%s" value="0" /><input type="hidden" name="StockID%s" value="%s" /></td>
						</tr>',
						$MyRow['stockid'],
						$MyRow['description'],
						$MyRow['units'],
            ...
            echo '<tr>
				...
				<tr>
					<td><input type="hidden" name="previous" value="' . strval($Offset-1) . '" /><input tabindex="'.strval($tabidx++).'" type="submit" name="Prev" value="' . _('Prev') . '" /></td>
					<td class="centre" colspan="6"><input type="hidden" name="SelectingOrderItems" value="1" /><input tabindex="'.strval($tabidx++).'" type="submit" value="'._('Add to Sale').'" /></td>
					<td><input type="hidden" name="NextList" value="'.strval($Offset+1).'" /><input tabindex="'.strval($tabidx++).'" type="submit" name="Next" value="'._('Next').'" /></td>
				</tr>
				</table>
			...
			mostrarAyuda('Use this form to add items quickly if the item codes are already known');
		...
		for ($i=1;$i<=$_SESSION['QuickEntries'];$i++) {
			...
			if($i==$_SESSION['QuickEntries']){
	 		...
	 		echo '<td><input type="text" name="part_' . $i . '"' . ($i==1 ? ' autofocus="autofocus"':'') . ' data-type="no-illegal-chars" 
			 		class="entryCode"
