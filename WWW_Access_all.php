...
if (isset($_POST['submit']) OR isset($_POST['moveBtn']) ) {
	...
	if (isset($_POST['SecRoleName']) ){ // Update or Add Security Headings
		...
		$PageTokenId = $_POST['PageToken'];
		$sql = "SELECT * from securitygroups 
					WHERE secroleid='".$SelectedRole."' AND
						tokenid='".$PageTokenId."'";
		$ErrMsg = _('The access of the page group failed because');
		$ResMsg = _('OK');
		$result = DB_query($sql,$ErrMsg);
		if(DB_num_rows($result)==0 ) { // updating Security Groups add a page token			
		...
		} else { // updating Security Groups remove a page token
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
echo '<input type="hidden" name="PageToken"  id="PageToken"  value="" />';
if( isset($_POST['SelectedRole'])) {
	echo '<input type="hidden" name="SelectedRole" value="' . $_POST['SelectedRole'] . '" />';
}
	...
	while($AvailRow = DB_fetch_array($Result)) {
		...
		if (in_array($AvailRow['tokenid'],$TokensUsed)){
			printf('<tr class="striped_row">
					<td>%s</td>
					<td>%s</td>
					<td><a href="#" onclick="sendMove(\'%s\');">' . _('Remove') . '</a></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					</tr>',
			...
			printf('<tr class="striped_row">
					...
					<td>%s</td>
					<td>%s</td>
					<td><a href="#" onclick="sendMove(\'%s\');">' . _('Add') . '</a></td>
					</tr>',
					$AvailRow['tokenid'],
					$AvailRow['tokenname'],
					$AvailRow['tokenid'] );
	...
	echo '</table>
		<input type="submit" id="moveBtn" name="moveBtn" value="" style="display:none;"/>
	</form>';
}
?>
<script type="text/javascript">
	function sendMove(tokenId){
		document.getElementById('PageToken').value=tokenId;
		document.getElementById('moveBtn').click();
		return confirm('<?=_('Are you sure you wish to delete this security token from this role?');?>')
	}
</script>
<?php
