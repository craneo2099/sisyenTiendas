...
if ( 
	isset($_POST['DoIt'])   AND (
		$_POST['PrintOrEmail'] == 'Print' OR $ViewingOnly == 1
	)
) {
	$MakePDFThenDisplayIt = True;
...
if(isset($_POST['refresh'])){
	unset($MakePDFThenDisplayIt);
	unset($MakePDFThenEmailIt);
}
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	if ($MakePDFThenDisplayIt) {
		$pdf->OutputD($_SESSION['CompanyRecord']['coyname'] . '_OrdenCompra_' .$OrderNo. '_' . date('Y-m-d') . '.pdf');
		$pdf->__destruct();
	} else {
		/* must be MakingPDF to email it */

		$PdfFileName = $_SESSION['CompanyRecord']['coyname'] . '_OrdenCompra_' . $OrderNo . '_' . date('Y-m-d') . '.pdf';
	...
	if ($_POST['PrintOrEmail'] == 'Email') {
		...
		if (DB_num_rows($ContactsResult) > 0) {
			...
			while ($ContactDetails = DB_fetch_array($ContactsResult)) {
				$SupplierID=$ContactDetails['supplierid'];
			...
			echo '</select><a href="' . $RootPath . '/SupplierContacts.php?SupplierID=' . $SupplierID . '"
			target="_blank">',_('Add'),'</a><a href="#" onclick="ReloadForm(form1.refresh)">
			',_('Update'),'
			</a></td></tr></table>';
		} else {
			echo '</table><br />';
			$SQL = "SELECT purchorders.supplierno
				FROM purchorders
				WHERE purchorders.orderno='" . $OrderNo . "'";
			$SupplierID = DB_fetch_array(DB_query($SQL, $ErrMsg))[0];
			prnMsg(_('There are no contacts defined for the supplier of this order') 
			. '. ' . _('You must first set up supplier contacts before emailing an order')
			. ' <a href="' . $RootPath . '/SupplierContacts.php?SupplierID=' . $SupplierID . '"
			target="_blank">'._('Add').'</a><a href="#" onclick="ReloadForm(form1.refresh)">
			'._('Update').'
			</a>', 'error');
			echo '<br />';
