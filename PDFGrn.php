...
if ($NoOfGRNs >0){
	...
	for ($i=1;$i<=$NoOfGRNs;$i++) {
		...
		$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column5->x,$Page_Height-$YPos,$FormDesign->Data->Column5->Length,$FormDesign->Data->Column5->FontSize, _($myrow['suppliersunit']), 'left');
		...
		$LeftOvers = $pdf->addTextWrap($FormDesign->Data->Column7->x,$Page_Height-$YPos,$FormDesign->Data->Column7->Length,$FormDesign->Data->Column7->FontSize, _($myrow['units']), 'left');
		$YPos += $line_height;
    ...
    $pdf->OutputD($_SESSION['CompanyRecord']['coyname']. '_NBR_' . $GRNNo . '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
