...
foreach ($TaxCategoryName as $TaxCategoryId => $Row) {
	...
	echo ' value="' . $TaxCategoryId . '">' . _($Row) . '</option>';
}
echo '</select></td></tr>';

// Recovery or Stock GL Code input.
echo '<input type="submit" name="UpdateTypes" style="visibility:hidden;" value="Not Seen" />';
//Se eliminan campos de administracion de contabilidad de-no
echo '<input type="hidden" name="StockAct" value="1010"/>
<input type="hidden" name="WIPAct" value="1010"/>
<input type="hidden" name="AdjGLAct" value="1" />
<input type="hidden" name="IssueGLAct" value="1" />
<input type="hidden" name="PurchPriceVarAct" value="1" />
<input type="hidden" name="MaterialUseageVarAc" value="1" />';
