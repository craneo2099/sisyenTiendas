...
/* if update quantities button is hit page has been called and ${$Line->LineNo} would have be
 set from the post to the quantity to be received */
	...
	foreach ($poid->LineItems as $Line) {
		$RecvQty = round(filter_number_format($_POST['RecvQty_' . $Line->LineNo]),$Line->DecimalPlaces);
		if (!is_numeric($RecvQty)) {
			$RecvQty = 0;
		}
		$poid->LineItems[$Line->LineNo]->ReceiveQty = $RecvQty;
		if (isset($_POST['Complete_' . $Line->LineNo])) {
			$poid->LineItems[$Line->LineNo]->Completed = 1;
		} else {
			$poid->LineItems[$Line->LineNo]->Completed = 0;
		}
	}
}

if ($poid->Status != 'Printed') {
	prnMsg( _('Purchase orders must have a status of Printed before they can be received').'.<br />' .
		_('Order number') . ' ' . $_GET['PONumber'] . ' ' . _('has a status of') . ' ' . _($poid->Status), 'warn');
	include('includes/footer.php');
	exit;
}
...
echo '<p class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
	...
	_('Receive Purchase Order'), ' : ', $poid->OrderNo, ' ', _('from'), ' ', $poid->SupplierName, '</p>';// Page title.
...
if (count($poid->LineItems)>0 and !isset($_POST['ProcessGoodsReceived'])) {

	foreach ($poid->LineItems as $LnItm) {
...
maxlength="10" size="10" value="' . locale_number_format(round($LnItm->ReceiveQty,$LnItm->DecimalPlaces),$LnItm->DecimalPlaces) . '" /></td>';
	...
	$DisplayTotal = locale_number_format($poid->Total,$poid->CurrDecimalPlaces);
	if ($_SESSION['ShowValueOnGRN'] == 1) {
		echo '<tr>
...
if (count($poid->LineItems)>0) {

	foreach ($poid->LineItems as $OrderLine) {
...
if ($poid->SomethingReceived()==0 AND isset($_POST['ProcessGoodsReceived'])) { /*Then dont bother proceeding cos nothing to do ! */
...
}  elseif (isset($_POST['ProcessGoodsReceived']) AND $poid->SomethingReceived()==1 AND $InputError == false) {
...
// Otherwise if you try to fullfill item quantities separately will give error.
	$SQL = "SELECT itemcode,
			...
			WHERE orderno='" . (int) $poid->OrderNo . "'
			AND completed=0
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($poid->LineItems[$LineNo]->GLCode != $myrow['glcode'] OR
			$poid->LineItems[$LineNo]->ShiptRef != $myrow['shiptref'] OR
			$poid->LineItems[$LineNo]->JobRef != $myrow['jobref'] OR
			$poid->LineItems[$LineNo]->QtyInv != $myrow['qtyinvoiced'] OR
			$poid->LineItems[$LineNo]->StockID != $myrow['itemcode'] OR
			$poid->LineItems[$LineNo]->Quantity != $myrow['quantityord'] OR
			$poid->LineItems[$LineNo]->QtyReceived != $myrow['quantityrecd']) {
			...
			if ($debug==1) {
				echo '<table class="selection">
					<tr>
						...
						<td>' . $poid->LineItems[$LineNo]->GLCode . '</td>
						<td>' . $myrow['glcode'] . '</td>
					</tr>
					<tr>
						<td>' . _('ShiptRef of the Line Item') . ':</td>
						<td>' . $poid->LineItems[$LineNo]->ShiptRef . '</td>
						<td>' . $myrow['shiptref'] . '</td>
					</tr>
					<tr>
						<td>' . _('Contract Reference of the Line Item') . ':</td>
						<td>' . $poid->LineItems[$LineNo]->JobRef . '</td>
						<td>' . $myrow['jobref'] . '</td>
					</tr>
					<tr>
						<td>' . _('Quantity Invoiced of the Line Item') . ':</td>
						<td>' . locale_number_format($poid->LineItems[$LineNo]->QtyInv,$poid->LineItems[$LineNo]->DecimalPlaces) . '</td>
						<td>' . $myrow['qtyinvoiced'] . '</td>
					</tr>
					<tr>
						<td>' . _('Stock Code of the Line Item') . ':</td>
						<td>' .  $poid->LineItems[$LineNo]->StockID . '</td>
						<td>' . $myrow['itemcode'] . '</td>
					</tr>
					<tr>
						<td>' . _('Order Quantity of the Line Item') . ':</td>
						<td>' . locale_number_format($poid->LineItems[$LineNo]->Quantity,$poid->LineItems[$LineNo]->DecimalPlaces) . '</td>
						<td>' . $myrow['quantityord'] . '</td>
					</tr>
					<tr>
						<td>' . _('Quantity of the Line Item Already Received') . ':</td>
						<td>' . locale_number_format($poid->LineItems[$LineNo]->QtyReceived,$poid->LineItems[$LineNo]->DecimalPlaces) . '</td>
						<td>' . locale_number_format($myrow['quantityrecd'],$poid->LineItems[$LineNo]->DecimalPlaces) . '</td>
					</tr>
					</table>';
			}
			echo '<div class="centre"><a href="' . $RootPath . '/PO_SelectOSPurchOrder.php">' .
				_('Select a different purchase order for receiving goods against') . '</a></div>';
			echo '<div class="centre"><a href="' . $RootPath . '/GoodsReceived.php?PONumber=' . $poid->OrderNo . '">' .  _('Re-read the updated purchase order for receiving goods against'). '</a></div>';
			unset($poid->LineItems);
			unset($poid);
			unset($_POST['ProcessGoodsReceived']);
            echo '</div>';
...
/*Now Get the next GRN - function in SQL_CommonFunctions*/
	...
	foreach ($poid->LineItems as $OrderLine) {
		...
		if ($OrderLine->ReceiveQty !=0 AND $OrderLine->ReceiveQty!='' AND isset($OrderLine->ReceiveQty)) {
			...
			$LocalCurrencyPrice = ($OrderLine->Price / $poid->ExRate);
			...
			if ($OrderLine->StockID!='') { //Its a stock item line
				...
				if($myrow[1] != 'D') {
					if ($OrderLine->QtyReceived==0) { //its the first receipt against this line
						$poid->LineItems[$OrderLine->LineNo]->StandardCost = $myrow[0];
					}
					...
					/*Set the purchase order line stdcostunit = weighted average / standard cost used for all receipts of this line
						...
						$poid->LineItems[$OrderLine->LineNo]->StandardCost = (($CurrentStandardCost * $OrderLine->ReceiveQty) + ($poid->LineItems[$OrderLine->LineNo]->StandardCost * $OrderLine->QtyReceived)) / ($OrderLine->ReceiveQty + $OrderLine->QtyReceived);
				} elseif ($myrow[1] == 'D') { //it's a dummy part which without stock.
					$Dummy = true;
					if($OrderLine->QtyReceived == 0) {//There is
						$poid->LineItems[$OrderLine->LineNo]->StandardCost = $LocalCurrencyPrice;
					}
				}
			...
			if ($OrderLine->StockID=='' OR !empty($Dummy)) { /*Its a NOMINAL item line */
				$CurrentStandardCost = $poid->LineItems[$OrderLine->LineNo]->StandardCost;
			}
...
/*Now the SQL to do the update to the PurchOrderDetails */
			...
			if ($OrderLine->ReceiveQty >= ($OrderLine->Quantity - $OrderLine->QtyReceived)) {
				$SQL = "UPDATE purchorderdetails SET quantityrecd = quantityrecd + '" . $OrderLine->ReceiveQty . "',
													stdcostunit='" . $poid->LineItems[$OrderLine->LineNo]->StandardCost . "',
													completed=1
				...
				$SQL = "UPDATE purchorderdetails SET
												...
												stdcostunit='" . $poid->LineItems[$OrderLine->LineNo]->StandardCost . "',
												completed='" . $poid->LineItems[$OrderLine->LineNo]->Completed . "'
										WHERE podetailitem = '" . $OrderLine->PODetailRec . "'";
			}
			...
			if ($OrderLine->StockID !='' AND !isset($Dummy)) { /*Its a stock item so use the standard cost for the journals */
				...
				$UnitCost = $OrderLine->Price / $poid->ExRate;
			}
...
/* Need to get the current location quantity will need it later for the stock movement */
	...
	/* Insert stock movements - with unit cost */
				...
				$SQL = "INSERT INTO stockmoves (stockid,
									...
									VALUES (
										...
										'" . $LocalCurrencyPrice . "',
										'" . $PeriodNo . "',
										'" . $poid->SupplierID . " (" . DB_escape_string($poid->SupplierName) . ") - " .$poid->OrderNo . "',
										'" . $OrderLine->ReceiveQty . "',
			...
			if ($OrderLine->AssetID !='' AND $OrderLine->AssetID !='0') { //then it is an asset
				...
				if (DB_num_rows($CheckAssetExistsResult)==1) { //then work with the assetid provided
					...
					$poid->LineItems[$OrderLine->LineNo]->GLCode = $AssetRow['costact'];
			...
			if ($poid->GLLink==1 AND $OrderLine->GLCode !=0) {
				...
				$SQL = "INSERT INTO gltrans (type,
									...
									VALUES (
											...
											$PeriodNo . "','" .
											$OrderLine->GLCode . "','" .
											_('PO') . ' ' . $poid->OrderNo . ': ' . $poid->SupplierID . ' - ' . $OrderLine->StockID . ' - ' . DB_escape_string($OrderLine->ItemDescription) . ' x ' . $OrderLine->ReceiveQty . " @ " . locale_number_format($CurrentStandardCost,$_SESSION['CompanyRecord']['decimalplaces']) . "','" .
											$CurrentStandardCost * $OrderLine->ReceiveQty . "')";
				...
				$SQL = "INSERT INTO gltrans (type,
									...
									VALUES (25,'" .
											...
											_('PO') . ' ' . $poid->OrderNo . ': ' . $poid->SupplierID . ' - ' . $OrderLine->StockID . ' - ' . DB_escape_string($OrderLine->ItemDescription) . ' x ' . $OrderLine->ReceiveQty . ' @ ' . locale_number_format($UnitCost,$_SESSION['CompanyRecord']['decimalplaces']) . "','" .
	...
	if ($poid->AllLinesReceived()==1 OR $OrderCompleted) { //all lines on the purchase order are now completed
		$StatusComment=date($_SESSION['DefaultDateFormat']) .' - ' . _('Order Completed on entry of GRN')  . '<br />' . $poid->StatusComments;
		$sql="UPDATE purchorders
				SET status='Completed',
				stat_comment='" . $StatusComment . "'
				WHERE orderno='" . $poid->OrderNo . "'";
		$result=DB_query($sql);
	}
...
/*end of process goods received entry */
	...
	echo '<br />
		<div class="centre">
			<a href="' . $RootPath . '/PO_Header.php?ModifyOrderNumber=' .$poid->OrderNo . '">' . _('Modify Order Items'). '</a>
		</div>
		<br />
		<div class="centre">
