...
}elseif(DB_num_rows($locResult)==1){
		$SelectedLocation=DB_fetch_array($locResult)['loccode'];
...
if (isset($_POST['submit'])) {
		...
		if ($CheckRow[0] > 0) {
			...
			$SQL = "INSERT INTO locationusers (loccode,
										...
										VALUES ('" . $_POST['SelectedLocation'] . "',
												...
												'1',
												'1')";
			$msg = sprintf(_('User: %s authority to use the %s location has been changed'),
				$_POST['SelectedUser'] , $_POST['SelectedLocation'] );
	...
	$msg = sprintf(_('User %s has had their authority to use the %s location removed'),
		$SelectedUser, $SelectedLocation );
	prnMsg($msg, 'success');
	unset($_GET['delete']);
	...
	$ErrMsg = _('The Location user record could not be deleted because');
	$Result = DB_query($SQL, $ErrMsg);
	$msg = sprintf(_('User %s has had their authority to update %s location removed'),
		$SelectedUser, $SelectedLocation);
	prnMsg($msg, 'success');
