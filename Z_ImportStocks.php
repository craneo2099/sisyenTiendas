...
$Title = _('Import Stock Items from .csv');
...
if (isset($_FILES['userfile']) and $_FILES['userfile']['name']) { //start file processing
	...
	foreach ($headRow as $headField) {
		if ( mb_strtoupper($headField) != mb_strtoupper($FieldHeadings[$head]) ) {
			prnMsg (sprintf(_('File contains incorrect headers %u  != . Try downloading a new template.'),mb_strtoupper($headField),mb_strtoupper($FieldHeadings[$head])),'error');  //Fixed $FieldHeadings from $headings
			fclose($FileHandle);
			include('includes/footer.php');
			exit;
		}
	...
	while ( ($myrow = fgetcsv($FileHandle, 10000, ",")) !== FALSE ) {
		...
		if ($testrow[0] != 0) {
			$InputError = 1;
			prnMsg (sprintf(_('Stock item %u already exists'),$StockID),'error');
		}
	...
	if ($InputError == 1) { //exited loop with errors so rollback
		prnMsg(_('Failed on row %u. Batch import has been rolled back.'),'error');
		DB_Txn_Rollback();
	...
	echo '<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') .
	 '">'._('Return').'</a>';
	...
	echo '
		<br />
		<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?gettemplate=1">'._('Get Import Template').'</a>
		<br />
		<br />';
