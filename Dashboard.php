...
include('includes/devstar/dashboard.inc');
...
if(isset($_POST['registrar'])){
	registrarEntradaSalida($_POST['selSalesP']);
}
...
$showDebtor=in_array($DebtorSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($DebtorSecurity);
...
$filter=[];
$filter['current']=1;
$salesPeopleList=getSalesmanFilter('',$filter);
...
$filter=[];
$filter['inicio']=date("Y-m-d");;
$checks=getCheckSalespList($filter);
	?>
	<p class="page_title_text"><img alt="" src="<?=$RootPath?>/css/<?=$Theme?>/images/gl.png"
		title="<?=$Title?>" />
		<?=$Title?></p>
	<div class="col-12 clock">
		<h2><span id="hours" class="hours"><?=date("H");?></span> :
		    <span id="minutes" class="minutes"><?=date("i");?></span> :
		    <span id="seconds" class="seconds"><?=date("s");?></span></h2>
	</div>
	<script type="text/javascript">
		function clockEngine() {
			var horas=parseInt($('#hours').text()),
				minutos=parseInt($('#minutes').text()),
				segundos=parseInt($('#seconds').text())+1;
		if(segundos==60){
			segundos=0;
			minutos++;
		}		
	...
	<?php //Seccion de checador de vendedores ?>
	<div class="col-6 offset-3 selection">
		<form method="post" action="<?=$formAction?>">
			<input type="hidden" name="FormID" value="<?=$_SESSION['FormID']?>" />
			<div class="form-group row">
				
		      <label for="selSalesP" class="col-sm-2 col-form-label"><?=_('Salesman')?></label>
		      <div class="col-sm-7">
		      	<select class="custom-select" id="selSalesP" name="selSalesP">
		    <?php
		     foreach ($salesPeopleList as $salesPeople) {
		    ?>
		        <option value="<?=$salesPeople['salesmancode']?>"><?=$salesPeople['salesmanname']?></option>
		    <?php }
			?>
			      </select>
			  </div>
		      <button type="submit" name="registrar" class="btn btn-primary col-sm-3"><?=_('Register')?></button>
		    </div>
		</form>
		<?php
		if($checks){ ?>
		<table class="table table-hover table-sm min-w-100">
			<caption><?=_('Last salespeople access records')?></caption>
			<thead>
				<tr>
					<th scope="col"><?=_('Salesman')?></th>
					<th scope="col"><?=_('Clock on')?></th>
					<th scope="col"><?=_('Clock out')?></th>
				</tr>
			</thead>
			<tbody>
				<?php 
	...
	if($showDebtor){
		?>
	<div class="col-12">
		<h2><?= _('Overdue Customer Balances')?></h2></div>
		<div class="col-12">
...
<?php
	...
	$TotOD2 = 0;
	try{
		$CustomerResult=saldosVencidos($_POST['Salesman']);
	}catch(Exception $e){
...
if(in_array($PayeeSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($PayeeSecurity)) {
	echo '<div class="col-12">
		<h2>', _('Supplier Invoices Due within 1 Month'), '</h2></div>
		<div class="col-12">
		<table class="selection">
		<thead>
			<tr>
...
if(in_array($CashSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($CashSecurity)) {
	...
	echo '<div class="col-12">
		<h2>', _('Bank and Credit Card Balances'), '</h2></div>
		<div class="col-12">
		<table class="selection">
		<thead>
			<tr>
...
if(in_array($OrderSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($OrderSecurity)) {
	echo '<div class="col-12">
		<h2>', _('Outstanding Orders'), '</h2></div>
		<div class="col-12">
		<table class="selection">
		<thead>',
			'<tr>
