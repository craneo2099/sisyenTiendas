...
function rLocaleNumber() {
	...
	switch(Lang) {
		...
		case 'GM':
			...
			break;
		default:
			alert('Algo esta mal con la configuración de tu idioma');
	}
	if(patt.test(this.value)) {
		...
		this.setCustomValidity('El formato de número es incorrecto');
		return false;
	}
}
...
function isDate(dS, dF) {
	...
	if(mA==null) {
		alert("Por favor, ingrese la decha en el formato "+dF);
		return false;
	}
...
function initial() {
	...
	$("input.number").on("focus",function (){$(this).select()});
}
